...
class Water:
	def __init__(self,top,level_width):
		...
		self.__water_sprites = pygame.sprite.Group()

		for tile in range(tile_x_amount):
			x = tile * water_tile_width + water_start
			y = top
			sprite = AnimatedTile(192,x,y,'graphics/decoration/water')
			self.__water_sprites.add(sprite)
		
	def get_water_sprite(self):
		return(self.__water_sprites)

	def draw(self,surface,shift):
		self.get_water_sprite().update(shift)
		self.get_water_sprite().draw(surface)
...
class Clouds:
	def __init__(self,horizon,level_width,cloud_number):
		...
		min_y = 0
		max_y = horizon
		self.__cloud_sprites = pygame.sprite.Group()
		...
		for cloud in range(cloud_number):
			...
			self.__cloud_sprites.add(sprite)

	def get_cloud_sprite(self):
		return(self.__cloud_sprites)

	def draw(self,surface,shift):
		self.get_cloud_sprite().update(shift)
		self.get_cloud_sprite().draw(surface)
